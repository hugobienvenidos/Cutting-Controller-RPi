...
def _read_onoff_status(client: ModbusSerialClient, addr: int, mirror_cfg: dict) -> bool | None:
    ...
    if rtype == "holding":
        res = client.read_holding_registers(offset, count=1, slave=addr)
        return bool(res.registers[0]) if not res.isError() else None
    if rtype == "input":
        res = client.read_input_registers(offset, count=1, slave=addr)
        return bool(res.registers[0]) if not res.isError() else None
    if rtype == "coil":
        res = client.read_coils(offset, count=1, slave=addr)
        return res.bits[0] if not res.isError() else None
    if rtype == "discrete":
        res = client.read_discrete_inputs(offset, count=1, slave=addr)
        return res.bits[0] if not res.isError() else None
...
def _read_vfd(client: ModbusSerialClient, addr: int) -> dict | None:
    ...
    r = VFD_REGISTERS
    regs = client.read_holding_registers(r["holding_start"], count=r["holding_count"], slave=addr)
    if regs.isError():
        return None
    return {
...
def write_vfd_speed(client: ModbusSerialClient, addr: int, speed_rpm: int):
    client.write_register(VFD_REGISTERS["speed_offset"], speed_rpm, slave=addr)


def write_vfd_onoff(client: ModbusSerialClient, addr: int, state_on: bool):
    client.write_register(VFD_REGISTERS["onoff_offset"], 1 if state_on else 0, slave=addr)